...
export default function StoreLocationPicker() {
    ...
    // Fetch API Key from server endpoint to prevent exposing it directly in the client bundle
    useEffect(() => {
        fetch("/api/maps-key")
            .then((res) => res.json())
            .then((data) => {
    ...
    }, [isMapLoaded, mapContainerRef.current, mapsApiKey]);
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            {/* Load the Google Maps script dynamically when key is available */}
            {mapsApiKey && (
                <Script
                    src={`https://maps.googleapis.com/maps/api/js?key=${mapsApiKey}&libraries=geometry,places`}
