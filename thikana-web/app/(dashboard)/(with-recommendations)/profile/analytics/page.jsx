...
import { ChevronLeft, FileDown, ChartBar } from "lucide-react";
import { saveAs } from "file-saver";
import html2canvas from "html2canvas";

// Text-based PDF report generation
const generatePDF = async (userAnalytics, productsWithRevenue, monthlyAnalytics, yearlyAnalytics) => {
  let toastId = null;
  try {
    ...
    return (
      <div className="container mx-auto p-8 text-center">
        <div className="flex flex-col items-center justify-center py-16 bg-gray-50 rounded-3xl border border-gray-100">
          <ChartBar className="w-16 h-16 text-muted-foreground/30 mb-4" />
          <h2 className="text-2xl font-bold text-gray-900 mb-2">No Analytics Yet 📊</h2>
          <p className="text-gray-600 mb-6">Start selling products to see your analytics here.</p>
          <Link href="/profile/inventory">
            <Button variant="outline"><ChevronLeft className="mr-2" /> Go to Inventory</Button>
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex items-center justify-between gap-4 mb-6">
        ...
        <Button
          onClick={() => generatePDF(userAnalytics, productsWithRevenue, monthlyAnalytics, yearlyAnalytics)}
