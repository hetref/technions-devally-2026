...
export async function POST(req) {
  try {
    ...
    // You must provide at least one way to dial out: a VAPI phone number ID, OR Twilio credentials
    const vapiPhoneNumberId = process.env.VAPI_PHONE_NUMBER_ID;
    const twilioSid = process.env.TWILIO_ACCOUNT_SID;
    const twilioToken = process.env.TWILIO_AUTH_TOKEN;

    if (!vapiPhoneNumberId && (!twilioSid || !twilioToken)) {
      return NextResponse.json(
        {
          error: "Missing Outbound Phone Configuration",
          details:
            "You must set either VAPI_PHONE_NUMBER_ID (if using a number purchased via Vapi) OR TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN in your .env.local file.",
        },
        { status: 400 }
      );
    }

    if (voiceConfig.voiceId === "bobby") {
      voiceConfig.voiceId = "Hmz0MdhDqv9vPpSMfDkh";
    }
...
Remember to be helpful and represent the business professionally.`,
    ...
    const callPayload = {
      ...
      ...(vapiPhoneNumberId 
        ...
        : {
            phoneNumber: {
              twilioPhoneNumber: process.env.TWILIO_PHONE_NUMBER || "+17622545834",
