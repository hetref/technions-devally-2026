...
export default function CallAutomationSettings() {
  ...
  const handleMakeCall = async (data) => {
    try {
      ...
      // Record the call in Firebase so it appears in the History tab
      await addDoc(collection(db, "users", user.uid, "requestCalls"), {
        ...
        callType: script.callType,
        status: "initiated",
        callId: responseData.id || responseData.call_id || responseData.assistantId,
