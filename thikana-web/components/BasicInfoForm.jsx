...
const basicInfoSchema = z.object({
  ...
  // added optional fields for GST type and state code
...
export default function BasicInfoForm() {
  ...
  // state name -> code mapping (normalized lowercase keys)
  const STATES_MAP = {
    "jammu and kashmir": "1",
    "jammu & kashmir": "1",
    "himachal pradesh": "2",
  ...
  // normalize and attempt to find a state code inside an address string
  const determineStateCodeFromString = (text) => {
    if (!text) return null;
    const lower = text.toLowerCase();
    // try to match any key
    for (const [name, code] of Object.entries(STATES_MAP)) {
      if (lower.includes(name)) {
        return code;
      }
    }
    // also try matching short names (e.g., "maharashtra" already covered);
    // if no match, return null
    return null;
  };

  // Reverse geocode using Nominatim if only lat/lon available (best-effort)
  const reverseGeocodeToState = async (lat, lon) => {
    try {
      ...
      const stateFromApi =
        (json.address && (json.address.state || json.address.county)) || null;
      if (!stateFromApi) return null;
      return determineStateCodeFromString(stateFromApi);
    } catch (err) {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          // determine state_code:
          if (userData.state_code) {
            form.setValue("state_code", String(userData.state_code));
          } else {
            // try address fields from userData
            const addressSource =
              userData.locationAddress || userData.address || "";
            let code = determineStateCodeFromString(addressSource);
            // if not found and lat/lon present, try reverse geocode
            if (!code && userData.location && (userData.location.latitude || userData.location.lat)) {
              const lat = userData.location.latitude ?? userData.location.lat;
              const lon = userData.location.longitude ?? userData.location.lng ?? userData.location.lon;
              if (lat && lon) {
                code = await reverseGeocodeToState(lat, lon);
              }
            }
            if (code) {
          ...
          // set gst_type if present
          if (userData.gst_type) {
            form.setValue("gst_type", userData.gst_type);
          }
        }
      } catch (error) {
  ...
  // watch address changes and attempt to set a default state_code when address updates
  const watchedAddress = form.watch("address");
  useEffect(() => {
    if (!watchedAddress) return;
    const code = determineStateCodeFromString(watchedAddress);
    if (code) {
      form.setValue("state_code", code);
    }
    // do not attempt reverse geocode on every keystroke
    // if no direct match, leave it to user or to existing DB value
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [watchedAddress]);
  ...
  async function updateProfile(userData) {
    try {
      ...
      // include gst_type and state_code in business doc as well if provided
      if (userData.gst_type) {
        businessData.gst_type = userData.gst_type;
      }
      if (userData.state_code) {
        businessData.state_code = userData.state_code;
      }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormItem className="md:col-span-2">
                  ...
                  <FormControl>
                    <select
                      {...field}
                      disabled={isSubmitting}
                      className="w-full bg-gray-50 border rounded px-2 py-2"
                    >
                      <option value="">Select GST Type (optional)</option>
            ...
            {/* State Code field - auto-determined but user can change */}
            <FormField
              control={form.control}
              name="state_code"
              render={({ field }) => (
                <FormItem className="md:col-span-2">
                  ...
                  <FormControl>
                    <select
                      ...
                      className="w-full bg-gray-50 border rounded px-2 py-2"
                    >
                      <option value="">Auto-detected or select state</option>
                      {Object.entries(STATES_MAP).map(([name, code]) => (
                  ...
                  <FormDescription>
                    State code is auto-determined from address/location but you
                    can change it here.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
